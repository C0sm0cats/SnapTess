...
export class Studio {
    constructor(extension) {
        ...
        for (const [label, style] of [['Active', 'active'], ['Scaled', 'scaled'], ['Floating', 'floating'], ['Empty', 'empty']]) {
    ...
    render() {
        ...
        rects.forEach((rect, index) => {
            ...
            if (w) {
                ...
                if (rect.height > 95) content.add_child(app?.create_icon_texture(36) ?? new St.Icon({icon_name: 'application-x-executable-symbolic', icon_size: 36}));
                content.add_child(new St.Label({text: app?.get_name() ?? 'Application', style_class: 'snaptess-app-name'}));
                ...
                card._delegate = {index, studio: this, getDragActor: () => {
                    ...
                    handleDragOut: () => card.remove_style_class_name('drop-target'),
                    acceptDrop: source => this.drop(source, index)};
                const draggable = DND.makeDraggable(card);
                draggable.connect('drag-begin', () => { this.selected = index; card.add_style_class_name('dragging'); });
                draggable.connect('drag-end', () => card.remove_style_class_name('dragging'));
            } else {
            ...
            if (this.animateSlots?.has(index)) {
