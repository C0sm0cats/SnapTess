...
export class Studio {
    constructor(extension) {
        ...
        this.saveButton = button('Save as layout…', () => {
            if (!this.creatingNew) this.previewSavedId = null;
    ...
    saveNamedLayout(replaceId = null) {
        const name = this.nameEntry.get_text().trim().slice(0, 60);
        const existing = this.extension.savedLayouts.find(item => item.name.toLowerCase() === name.toLowerCase());
        ...
        const id = this.creatingNew
            ? this.extension.saveNewLayout(name, this.newLayout.preset, this.newLayout.apps,
                this.newLayout.pinned, replaceId)
            : this.extension.saveLayout(name, this.preset, this.draft, this.pins, replaceId);
        if (!id) {
            this.nameEntry.set_hint_text('Enter a layout name');
            return;
        }
        this.nameEntry.set_text(''); this.nameRow.hide(); this.replaceRow.hide();
        if (this.creatingNew) { this.creatingNew = false; this.newLayout = null; this.newUndoStack = []; }
    ...
    renderSavedLayouts() {
        ...
        this.saveButton.visible = !saved && !this.showSavedChooser && !this.creatingNew;
        this.saveButton.label = this.creatingNew ? 'Save new layout…' : 'Save as layout…';
    ...
    undo() {
        if (this.creatingNew) {
            ...
            if (!draft) return;
            this.newLayout = draft; this.showLibrary = false; this.render();
            return;
        }
    ...
    choosePreset(id) {
        if (this.creatingNew) {
            ...
            if (id === 'auto' || !count || draft.apps.slice(count).some(Boolean)) return;
            if (id === draft.preset) { this.presetMenu?.close(); return; }
            this.saveNewUndo();
            draft.preset = id; draft.apps.length = count; draft.pinned.length = count;
    ...
    render() {
        ...
        this.dialog.setButtons(this.showSavedChooser ? [
            ...
            {label: 'Save layout…', default: true, action: () => {
                if (this.nameRow.visible) this.saveNamedLayout();
                else { this.nameRow.show(); this.nameEntry.grab_key_focus(); }
            }},
        ] : [
        ...
        if (this.width < 820) {
            const currentName = PRESETS.find(([id]) => id ===
                (this.creatingNew ? this.newLayout.preset : this.preset))?.[1] ?? 'Auto';
            ...
            for (const [id, name] of PRESETS) {
                if (this.creatingNew && id === 'auto') continue;
                const item = this.presetMenu.addAction(name, () => this.choosePreset(id));
                item.setSensitive(this.creatingNew ?
                    !this.newLayout.apps.slice(capacity(id)).some(Boolean) :
                    id === 'auto' || capacity(id) >= windows.length);
            }
        } else {
            ...
            for (const [id, name] of PRESETS) {
                ...
                if (this.creatingNew ? this.newLayout.apps.slice(capacity(id)).some(Boolean) :
                    id !== 'auto' && capacity(id) < windows.length) {
        ...
        rects.forEach((rect, index) => {
            ...
            if (w) {
                ...
                if (dense) content.add_child(app?.create_icon_texture(16) ??
                    new St.Icon({icon_name: 'application-x-executable-symbolic', icon_size: 16}));
                else if (this.showPreviews && actor && !minimal) {
    ...
    renderNewLayout(height) {
        const draft = this.newLayout, count = capacity(draft.preset);
        const rects = layout({x: 0, y: 0, width: this.width, height}, count,
            {preset: draft.preset, gap: 8, padding: 10,
        ...
        rects.forEach((rect, index) => {
            ...
            const dense = rect.height < 56;
            const card = button('', () => {
                draft.selected = index; this.showLibrary = !id; this.render();
            ...
            label.clutter_text.single_line_mode = true;
            content.add_child(label);
            if (draft.pinned[index] === id && id && !dense) {
                const badges = new St.BoxLayout({style_class: 'snaptess-state-badges'});
                badges.add_child(new St.Label({text: 'PINNED', style_class: 'snaptess-pinned-badge'}));
                content.add_child(badges);
            }
            card.accessible_name = `Tile ${index + 1}: ${name}${draft.pinned[index] ? ', pinned' : ''}`;
            card.set_child(content); this.canvas.add_child(card);
        });
        this.updatePinButton();
        this.hint.text = `NEW LAYOUT · ${draft.apps.filter(Boolean).length} apps · ${count} tiles · ` +
