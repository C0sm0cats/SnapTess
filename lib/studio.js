...
export class Studio {
    ...
    renderSavedPreview(saved, height) {
        ...
        rects.forEach((rect, index) => {
            const id = (saved.apps ?? saved.pinned)[index], w = plan.slots[index];
            ...
            if (id) {
                ...
                badges.add_child(new St.Label({text: w ? 'RUNNING' : 'WILL OPEN',
                    style_class: 'snaptess-saved-state'}));
                if (saved.pinned[index]) badges.add_child(new St.Label({text: 'PINNED',
