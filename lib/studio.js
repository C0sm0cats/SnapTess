...
export class Studio {
    constructor(extension) {
        ...
        this.pinButton = button('Keep this app in this tile', () => this.togglePin());
    ...
    render() {
        ...
        rects.forEach((rect, index) => {
            ...
            if (w) {
                ...
                if (this.pins[index] === this.extension.appId(w)) card.add_style_class_name('pinned');
        ...
        const selectedWindow = windows[this.selected];
        this.pinButton.visible = Boolean(selectedWindow);
        if (selectedWindow) {
            const pinned = this.pins[this.selected] === this.extension.appId(selectedWindow);
            this.pinButton.label = pinned ? 'Stop keeping this app in this tile' : 'Keep this app in this tile';
            this.pinButton[pinned ? 'add_style_class_name' : 'remove_style_class_name']('selected');
        }
        const windowCount = windows.filter(Boolean).length;
        this.hint.text = windowCount ? `${windowCount} windows · capacity ${count} · Drag to swap, or select two cards. Changes apply only when you confirm.` :
    ...
    select(index) {
        ...
        if (from === index) { this.selected = -1; this.render(); return; }
        this.saveUndo();
        this.selected = -1;
        [this.draft[from], this.draft[index]] = [this.draft[index], this.draft[from]];
        [this.pins[from], this.pins[index]] = [this.pins[index], this.pins[from]];
        this.markDirty();
